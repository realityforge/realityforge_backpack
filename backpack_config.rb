...
Backpack.organization('realityforge') do |o|
  ...
  o.repository('gwt-appcache-example', :description => 'A simple application demonstrating the use of the gwt-appcache library', :tags => %w(travis))
  o.repository('gwt-cache-filter', :description => 'A servlet filter that adds the appropriate http caching headers to GWT generated files based on *.cache.* and *.nocache.* naming patterns.', :issues => true, :tags => %w(travis))
  o.repository('gwt-cache-filter-example', :description => 'A simple application demonstrating the use of the gwt-cache-filter library', :tags => %w(travis))
  o.repository('gwt-contacts', :description => 'A version of the gwt "contacts" example from google', :tags => %w(travis))
  o.repository('gwt-datatypes', :description => 'A simple library that consolidates the common data types and associated infrastructure used across a range of GWT projects.', :tags => %w(travis))
  o.repository('gwt-eventsource', :description => 'GWT EventSource Library', :issues => true, :tags => %w(travis))
  o.repository('gwt-eventsource-example', :description => 'A simple application demonstrating the use of the gwt-eventsource library', :issues => true, :tags => %w(travis))
  o.repository('gwt-ga', :description => 'A simple GWT library for interacting with Google Analytics', :tags => %w(travis))
  o.repository('gwt-keycloak', :description => 'A simple library to provide keycloak support to GWT', :issues => true, :tags => %w(travis))
  o.repository('gwt-lognice', :description => 'A super simple gwt library that makes the log messages nicer.', :tags => %w(travis))
  o.repository('gwt-mmvp', :description => 'A micro MVP library that enhances the Activities and Places library.', :tags => %w(travis))
  o.repository('gwt-gin-extensions', :description => 'Simple utilities when using gin injection framework.', :tags => %w(travis))
  o.repository('gwt-property-source', :description => 'Provides a convenient way of compiling GWT property values into your module.', :issues => true, :tags => %w(travis))
  o.repository('gwt-property-source-example', :description => 'A sample application that demonstrates the use of the gwt-property-source', :issues => true, :tags => %w(travis))
  o.repository('gwt-webpoller', :description => 'A gwt library to simplify periodic polling and long-poll based transport layers', :issues => true, :tags => %w(travis))
  o.repository('gwt-webpoller-example', :description => 'A simple application demonstrating the use of the gwt-webpoller library', :issues => true, :tags => %w(travis))
  o.repository('gwt-websockets', :description => 'GWT WebSocket Library', :issues => true, :tags => %w(travis))
  o.repository('gwt-websockets-example', :description => 'A simple application demonstrating the use of the gwt-websockets library', :issues => true, :tags => %w(travis))
  ...
  o.repository('reality-mash', :description => 'A library providing the mash data type.', :tags => %w(travis))
  o.repository('reality-model', :description => 'Utility classes for defining a domain model.', :tags => %w(travis))
  o.repository('reality-naming', :description => 'A library to convert names between different naming conventions.', :tags => %w(travis))
  o.repository('reality-orderedhash', :description => 'A library providing a hash with preserved order and some array-like extensions.', :tags => %w(travis))
  o.repository('realityforge.github.io', :description => 'My personal website and blog', :homepage => 'https://realityforge.github.io')
  o.repository('replicant', :description => 'Client-side state representation infrastructure for GWT', :tags => %w(travis))
  o.repository('replicant-example', :description => 'A simple application demonstrating the use of the replicant library', :tags => %w(travis))
  o.repository('rest-criteria', :description => 'A simple library for parsing criteria in rest services', :tags => %w(travis))
  o.repository('rest-field-filter', :description => 'A simple library parsing field filters in rest APIs.', :tags => %w(travis))
  o.repository('simple-session-filter', :description => 'A simple servlet filter for implementing custom session management', :tags => %w(travis))
  o.repository('simple-keycloak-service', :description => 'A simple service interface and base classes to be used by keycloak secured services', :tags => %w(travis))
  o.repository('sqlshell', :description => 'A simple cross platform shell used to automate databases', :tags => %w(travis))
  o.repository('ssrs-api', :description => 'Generated SOAP interface to SSRS server', :tags => %w(travis))

  # Chef related code still in use until we get rid of chef
  o.repository('chef-archive', :description => 'Chef cookbook that provides utility LWRPs to download and unpack archives.', :tags => %w(travis))
  o.repository('chef-authbind', :description => 'A chef cookbook that installs/configures authbind and defines resources for managing authorization', :tags => %w(travis))
  o.repository('chef-cutlery', :description => 'Cutlery is a Chef cookbook containing a collection useful library code.', :tags => %w(travis))
  o.repository('chef-dbt', :description => 'Simple cookbook that aids in database migrations through dbt', :tags => %w(travis))
  o.repository('chef-glassfish', :description => 'A cookbook for managing GlassFish', :tags => %w(travis))
  ...
  o.repository('chef-hosts', :description => 'Chef cookbook to manage /etc/hosts file', :tags => %w(travis))
  o.repository('chef-kibana', :description => 'A chef cookbook that installs/configures kibana (the logstash UI)')
  o.repository('chef-sqlshell', :description => 'Simple cookbook to aid in automating database contents', :tags => %w(travis))
  o.repository('chef-winrm', :description => 'Simple winrm cookbook for chef', :tags => %w(travis))
  o.repository('chef-xymon', :description => 'A cookbook that installs the xymon monitoring software.', :tags => %w(travis))
  ...
  o.repository('knife-cookbook-doc', :description => 'Knife plugin to document cookbooks', :homepage => 'http://realityforge.github.com/knife-cookbook-doc', :tags => %w(travis))
  ...
  o.repository('geolatte-geom-eclipselink', :description => 'Converter for mapping Geolatte geometry types to eclipselink JPA provider', :tags => %w(deprecated travis))
  ...
  o.repository('gwt-online', :description => 'A gwt wrapper to determine when the browser is online', :tags => %w(historic))
  o.repository('gwt-online-example', :description => 'A simple application demonstrating the use of the gwt-online library', :tags => %w(historic))
  o.repository('gwt-packetio-example', :description => 'A sample application that demonstrates the use of the gwt-packetio library.', :tags => %w(historic))
